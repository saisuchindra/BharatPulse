...
/* ── Mini sparkline ─────────────────────────────────────────────── */
function Sparkline({ history = [], color = '#94a3b8', width = 140, height = 40 }) {
  ...
  return (
    <svg width={width} height={height} className="overflow-visible">
      <defs>
        <linearGradient id="silver-spark-fill" x1="0" y1="0" x2="0" y2="1">
...
/* ── Silver coin SVG icon ───────────────────────────────────────── */
function SilverIcon({ size = 20 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
      <circle cx="12" cy="12" r="10" fill="#94a3b8" opacity="0.15" stroke="#94a3b8" strokeWidth="1.5" />
      <circle cx="12" cy="12" r="7" fill="none" stroke="#94a3b8" strokeWidth="0.8" opacity="0.4" />
      <text x="12" y="16" textAnchor="middle" fill="#c0c8d4" fontSize="10" fontWeight="bold" fontFamily="monospace">Ag</text>
    </svg>
  );
}

export default function SilverPanel({ data }) {
  if (!data) return <SilverSkeleton />;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1.5">
        ...
        <span className="text-[8px] text-pulse-text-muted font-mono">XAG</span>
      </div>
      ...
      <div className={`p-2 rounded border ${bgAccent} ${breatheClass} mb-1.5`}>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-[8px] text-pulse-text-muted font-mono">INR/kg</div>
            <div className="text-xs font-bold font-mono text-slate-300 tabular-nums">
              ₹{data.inrKg?.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
            </div>
          </div>
        </div>
